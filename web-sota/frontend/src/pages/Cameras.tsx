			...
			<div className="flex max-w-[220px] flex-col items-center gap-1">
				...
				<div className="mt-2 flex gap-1">
					<Button
						...
						className="text-xs"
						disabled={busy}
						onClick={() => run(`/api/ptz/zoom-in/${enc}`)}
					>
						Zoom +
					</Button>
					<Button
						...
						className="text-xs"
						disabled={busy}
						onClick={() => run(`/api/ptz/zoom-out/${enc}`)}
					>
						Zoom −
					</Button>
				</div>
...
function statusLabel(cam: Camera): string {
	...
	if (s && typeof s === "object")
		return s.connected ? "online" : (s.error ?? "offline");
	return "unknown";
}
									...
									<CardContent className="space-y-3">
										...
										{(cam.type ?? "").toLowerCase() === "webcam" && (
											<p className="text-xs text-slate-500">
												USB: if the preview stays black, close Teams/Zoom/OBS
												using the camera, or stop the duplicate USB server on
												port 10715 so only one app captures the device.
											</p>
										)}
