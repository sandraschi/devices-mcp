...
export function Chat() {
  ...
  const loadProviders = async () => {
    try {
      ...
      if (data.success && data.providers?.length) {
        const names = data.providers.map((p: { type?: string; name?: string }) => p.type ?? p.name ?? '')
        setProviders(names)
  ...
  const loadModel = async () => {
    ...
    try {
      const r = await fetch('/api/llm/models/load', {
        ...
        body: JSON.stringify({ model_name: model, provider: provider || undefined }),
      })
      const data = await r.json()
  ...
  const send = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await r.json()
      if (r.ok && data.success) {
        const content = typeof data.response === 'string' ? data.response : (data.response ?? '')
        ...
        <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2 pb-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {providers.length > 0 && (
              <select
                value={provider}
                onChange={(e) => { setProvider(e.target.value); setModelLoaded(false) }}
            ...
            {models.length > 0 && (
              <select
                value={model}
                onChange={(e) => { setModel(e.target.value); setModelLoaded(false) }}
                className="rounded border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-600 dark:bg-slate-800"
              >
                {models.map((m) => (
            ...
            {model && (
              <Button size="sm" variant={modelLoaded ? 'outline' : 'default'} onClick={loadModel} disabled={loading}>
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : modelLoaded ? 'Loaded' : 'Load model'}
              </Button>
            )}
          </div>
        </CardHeader>
