...
Environment Variables:
    ...
    MCP_PORT: Port for HTTP/SSE. Default: 10716 (fleet 10700+; set MCP_PORT to override)
...
ENV_PORT = "MCP_PORT"  # default: 10716
...
def get_transport_config() -> dict:
    ...
    return {
        ...
        "port": int(os.getenv(ENV_PORT, "10716")),
...
Environment Variables:
  ...
  {ENV_PORT}         Port number (default: 10716)
...
Examples:
  ...
  # HTTP mode (web apps)
  python -m {server_name.replace("-", "_")} --http --port 10716

  # Via environment
  MCP_TRANSPORT=http MCP_PORT=10716 python -m {server_name.replace("-", "_")}
""",
    )
    ...
    parser.add_argument("--port", type=int, default=None, help=f"Port to listen on (default: ${ENV_PORT} or 10716)")
...
async def run_server_async(mcp_app, args: argparse.Namespace | None = None, server_name: str = "mcp-server") -> None:
    ...
    try:
        ...
        logger.exception("{server_name} failed:")
        raise
