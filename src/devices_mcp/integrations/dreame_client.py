        ...
        if self.token:
            try:
                ...
                async with self.session.get(f"{self.base_url}/api/states") as response:
                    ...
                    return {"success": False, "error": f"Failed to connect: HTTP {response.status}"}
        ...
        try:
            ...
            async with self.session.get(f"{self.dreame_mcp_url}/api/v1/health") as response:
                ...
                if not data.get("connected", False):
                    return {"success": False, "error": "dreame-mcp is reachable but not connected"}
                return {
                    "success": True,
    ...
    async def _call_service(
        ...
        try:
            ...
            if self._mode == "dreame_mcp":
                ...
                if not cmd:
                    return {"success": False, "error": f"Unsupported dreame-mcp command for service: {service}"}
